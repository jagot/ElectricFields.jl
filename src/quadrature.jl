...
change_interval(x::AbstractVector{T}, w::AbstractVector{T},
    ...
    change_interval!(similar(x,U), similar(w), x, w, a, b, γ)
...
basis of order `k`.
"""
function num_quadrature_points(ka, kb, k′)
    N2 = (ka-1) + (kb-1) + k′
    N2>>1 + N2&1
end
...
```
"""
function lgwt(ta::AbstractVector, ka,
              tb::AbstractVector, kb, N=num_quadrature_points(ka,kb,2))
    2N-1 ≥ (ka-1)+(kb-1) || @warn "N = $N quadrature point$(N > 1 ? "s" : "") not enough to calculate overlaps between polynomials of order ka,kb = $ka,$kb"
    x, w = gausslegendre(N)
...
lgwt(ta::AbstractKnotSet, tb::AbstractKnotSet, args...) =
    lgwt(ta, order(ta), tb, order(tb), args...)

lgwt(t::AbstractKnotSet, N::Integer) = lgwt(t, t, N)
