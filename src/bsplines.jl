...
support_interval(B::BSpline, j::Integer) =
    support_interval(knotset(B), j)
...
for f in (:knotset, :order)
    @eval $f(B::BSplineView) = $f(B.B)
end
...
support_interval(B::BSplineView, j::Integer) =
    support_interval(knotset(B), B.sel[j])
...
struct BasisAdjoint{Bt}
    B::Bt
end

Base.parent(B::BasisAdjoint) = B.B

LinearAlgebra.adjoint(B::BSplineOrView) = BasisAdjoint(B)

function Base.:(*)(A::BasisAdjoint{<:BSplineOrView}, B::BSplineOrView)
    At = parent(A)
    ...
    for i = 1:m
        for j = 1:n
            S[i,j] = dot(view(χA, :, i), W, view(χB, :, j))
        end
    end
