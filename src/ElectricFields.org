...
**** TEST Namespace macro
     #+BEGIN_SRC julia
       ...
       ElectricFields.@namespace!(params) do
           x = 5
           y = 3x
           z = x//3
           if x
       ...
       ElectricFields.@namespace!(params2) do
           if λ
               ν = 1/λ
           end
       end
...
* Dispersed fields
  ...
  #+BEGIN_SRC julia
    ...
    type DispersedField <: AbstractField
        a::AbstractField
        m::Medium
        d::Unitful.Length
    end
    ...
    function (f::DispersedField)(fs::Unitful.Frequency = default_sampling_frequency(f.a))
        F̂ = fft(f.a, fs)
        ifft(F̂.*dispersion(f.m, f.d, fftfreq(f.a, fs), frequency(f.a)))*u"V/m"
    end
