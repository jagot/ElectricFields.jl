...
* Field types
  #+BEGIN_SRC julia
    ...
    wavelength(f::LinearField) = wavelength(f.carrier)
    period(f::LinearField) = period(f.carrier)

    frequency(f::LinearField) = frequency(f.carrier)
    wavenumber(f::LinearField) = wavenumber(f.carrier)
    fundamental(f::LinearField) = fundamental(f.carrier)
    energy(f::LinearField) = energy(f.carrier)
...
** Gaussian
   ...
   #+BEGIN_SRC julia
     ...
     show(io::IO, env::GaussianEnvelope) =
         write(io, @sprintf("Gaussian envelope of duration %0.2f %s (intensity FWHM; ±%0.2fσ)",
                            usplit(env.τ)..., env.σmax))
...
**** Specifying wavelength
     ...
     :   Gaussian envelope of duration 6.20 fs (intensity FWHM; ±10.14σ)
...
**** Specifying period time
     ...
     :   Gaussian envelope of duration 10.00 s (intensity FWHM; ±7.07σ)
