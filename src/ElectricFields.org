...
** Linear field
   #+BEGIN_SRC julia
     ...
     intensity(f::LinearField) = intensity(envelope(f))
     amplitude(f::LinearField) = amplitude(envelope(f))
...
** Exports
   #+BEGIN_SRC julia
     export carrier,
         wavelength, period,
         frequency, wavenumber, fundamental, energy,
         envelope,
         intensity, amplitude,
         duration,
         params
...
** Gaussian
   ...
   #+BEGIN_SRC julia
     ...
     function GaussianEnvelope(field_params::Dict{Symbol,Any})
         ...
         @unpack τ, σ, σ′, σmax, σ′max, tmax, I₀, E₀ = field_params
         GaussianEnvelope(τ, σ, σ′, σmax, σ′max, tmax, I₀, E₀)
     end
   #+END_SRC
