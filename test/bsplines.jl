...
@testset "B-splines" begin
    ...
    @testset "Splines" begin
        ...
        @testset "Evaluate B-splines" begin
            ...
            @testset "Eval on subintervals" begin
                ...
                function testbasis(x)
                    ...
                    for j = 1:4
                        test_approx_eq(χ[:,j],B̃[:,j], atol=1e-15, rtol=1e-15)
                    end
                end
        ...
        @testset "Restricted basis" begin
            ...
            B̃ = B[:,2:end-1]
            @test size(B̃,2) == 6
            χ̃ = B̃[range(0, stop=1, length=10), :]
            @test size(χ̃,2) == 6
        end
        ...
        @testset "Function interpolation" begin
            ...
            χ = B[x̃,:]
            test_approx_eq(χ*c, sin.(2π*x̃), rtol=2e-7)

            B̃ = B[:,2:end-1]
            c̃ = B̃[x,:] \ sin.(2π*x)
            χ̃ = B̃[x̃,:]
            test_approx_eq(χ̃*c̃, sin.(2π*x̃), rtol=2e-7)
        end
    end
end
